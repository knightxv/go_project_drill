...
func PutUint64(key string, val uint64) (*clientv3.PutResponse, error) {
	etcdAddr := strings.Join(config.Config.Etcd.EtcdAddr, ",")
	cli, err := clientv3.New(clientv3.Config{
		Endpoints: strings.Split(etcdAddr, ","), DialTimeout: 5 * time.Second})
	...
	return cli.Put(context.Background(), GetPrefix(config.Config.Etcd.EtcdStorageSchema, key), strValue)
}
...
func GetUint64(key string) (uint64, error) {
	...
	//lease
	resp, err := cli.Get(context.Background(), GetPrefix(config.Config.Etcd.EtcdStorageSchema, key))
	if err != nil {
		return 0, err
	}
	for _, kv := range resp.Kvs {
		// kv.Value []byte 类型 转 uint64
